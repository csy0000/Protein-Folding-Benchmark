...
OpenFold should remain isolated in its own `openfold` environment. Existing benchmark score rows include prior single-sequence smoke outputs, but fresh runner invocations default to MSA mode. OpenFold MSA/template inference requires real parameter/database paths supplied by `OPENFOLD_REPO`, `OPENFOLD_PARAMS_DIR`, `OPENFOLD_DATA_DIR`, and related variables; the runner does not install dependencies or download databases automatically. Runtime exports are tracked in `envs/openfold_runtime.sh`.

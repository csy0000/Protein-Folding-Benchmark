...
OpenFold should remain isolated in its own `openfold` environment. The enabled smoke runner uses the local OpenFold checkout, AlphaFold-style parameters from `weights/colabfold/params`, and project-local smoke directories by default. Full OpenFold MSA/template inference still requires real parameter/database paths supplied by `OPENFOLD_REPO`, `OPENFOLD_PARAMS_DIR`, `OPENFOLD_DATA_DIR`, and related variables; the runner does not install dependencies or download databases automatically.

...
| `openfold` | `openfold` | yes | Existing scored outputs are from single-sequence smoke validation; fresh runner default is MSA mode. See `model-installation/openfold.md`. | `bash runners/run_openfold.sh data/sequences/1UAO_chignolin.fasta /tmp/openfold_test 1` |
...
OpenFold's existing score rows use AlphaFold-style parameters from
`weights/colabfold/params/params_model_1.npz` for a prior benchmark smoke path.
Fresh OpenFold runner calls default to MSA mode. Provide a real parameter
directory, template mmCIF directory, and MSA databases via the `OPENFOLD_*`
environment variables documented in `model-installation/openfold.md`.

...
| Protenix | `protenix` | `protenix` | `models/protenix` | experimental | yes | 7ROA and first-five CASP smoke | 1 | yes | PyPI `protenix==2.0.0` installed in a separate env with env-local CUDA toolkit 12.6 and cache/checkpoint under `weights/protenix`; the shared-MSA runner now defaults to model selector `protenix-v2`, and the shared ColabFold/MMseqs2 MSA smoke passed for 5/5 first-five targets. Disabled in canonical config. |
